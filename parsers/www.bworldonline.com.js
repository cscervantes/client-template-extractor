...
exports.ArticleLink = (data, cb) => {
    try{
        cdn.get(data.url, function(error, response, body){
        if(error){
            ...
            for(var i = 0; i < new_hrefs.length; i++){
                ...
                if(href.length < 4 || S(new_hrefs[i]).contains('/category/') || S(new_hrefs[i]).contains('/author/') || S(new_hrefs[i]).contains('facebook.com') || S(new_hrefs[i]).contains('twitter.com') || S(new_hrefs[i]).contains('instagram.com')){
                    continue
                }else{
  ...
  try{
    request(data.article_link, function(error, response, body){
      if(error){
        ...
        var ignoreSelector = [
            'div[class="td-post-header"] div.row:first-of-type', '*:empty', '.td-post-actions', '.td-post-featured-image', 'p:contains(By )',
          'div[class="td-post-header"]', 'div[id="billboard_article"]', '.td-loop-ads',
        ...
        var image = $('article figure > img').eq(0).attr('src')
